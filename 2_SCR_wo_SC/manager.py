...
class Manager(object):
    ...
    def test_encode_decode(self, filepath, file_idx, quality_level):
        ...
        string = self.sess.run(self.string_output,
                               feed_dict={self.pred_sigma: pred_sigma,
                                          self.y: y})
        ...
        y_hat = self.sess.run(self.symbols_output,
                              feed_dict={self.pred_sigma: pred_sigma, self.string_input: string})
        y_hat_entropy_decoding_time = time.time() - start_time

        start_time = time.time()
        recon = self.sess.run(self.recon_image, {self.y_hat: y_hat * IQV_q})
